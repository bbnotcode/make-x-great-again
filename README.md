...
| # | 想做的事 | 状态 | 简介 |
|---|---|:---:|---|
| **01** | **干掉刷评论的垃圾号** | ✅ Live | 扩展定时同步社区共建的公开黑名单；你已关注的账号优先放行，其余命中账号才会显示徽标并按设置处理（默认仅本地隐藏，可随时取消；也可选用你自己的 X 登录态做原生静音 / 拉黑）。名单由关键词规则 + AI 初筛、维护者人工确认后公开。 |
...
- **关注关系优先**：你在 X 上已关注的账号拥有最高放行优先级；即使账号出现在公开黑名单或命中检测规则，也不会被扩展标记、隐藏、静音或拉黑
- **本地徽标 + 一键隐藏**：命中名单的账号在推文旁出徽标，点「隐藏」本地隐藏该账号的帖子（5 秒可撤销）——**默认是纯本地隐藏，不调用 X 的任何接口**
- **帖子快捷静音 / 拉黑**：每条帖子右上角、Grok 按钮旁提供两个快捷按钮；无需先命中公榜或规则，即可明确选择用 X 原生功能静音或拉黑该账号。此处属于用户主动操作，因此不会被“已关注账号自动放行”拦截；按钮随 X 的虚拟列表即时挂载，返回上一页时不会延迟插入并挤动原生按钮
...
扩展会在每条普通帖子的 **Grok / 更多** 按钮旁增加两个小按钮：

- **扬声器带叉图标 — X 原生静音**：静音该账号，不取消关注，对方不会收到通知
- **盾牌带叉图标 — X 原生拉黑**：屏蔽该账号，并按 X 自身规则解除双方关注关系

这两个按钮不要求账号先命中公榜、关键词规则或检测缓存，适合处理漏网账号，或者你临时决定不想再看到的账号。点击后会直接使用浏览器中当前登录的 X 会话调用 X 自己的静音 / 拉黑接口；请求不经过 MXGA 服务端，也不会把帖子内容、账号凭据或操作记录上传给项目方。

快捷按钮属于**用户明确发起的操作**，因此与自动过滤的“关注账号优先保护”不同：即使你当前关注对方，只要主动点击快捷静音或拉黑，扩展仍会执行。为避免误触，扩展不会允许你对当前登录的自己执行操作；X 操作失败时，按钮会显示失败状态，帖子不会被伪装成已经成功处理。

所有原生静音 / 拉黑动作共用限速队列，跨标签页串行执行，并带有间隔、冷却、失败重试和 `429` 退避。短时间大量操作仍可能触发 X 的风控，建议优先静音，并分批处理。

### “我关注的人”为什么不会被公榜处理

**自动过滤时，你在 X 上的真实关注关系优先级最高。** 当账号命中公开名单、检测缓存、官方规则或个人正则时，扩展会先按以下顺序核实：

1. 使用当前页面已经显示的“正在关注”关系；
2. 使用近期从 X 核实并保存在本机的关注关系缓存；
3. 如果时间线帖子没有携带关注状态，再向 X 的只读关注关系接口核实一次。

只要 X 返回“正在关注”，扩展就会撤掉垃圾 / 色情徽标、恢复帖子显示，并取消尚未执行的自动隐藏、静音或拉黑。只读核实仅针对**已经命中过滤条件的账号**，不会遍历或上传你的完整关注列表；请求只发往 `x.com`，结果仅保存在 `chrome.storage.local`。已关注结果缓存 24 小时，未关注结果短期缓存 15 分钟，以兼顾请求数量和刚刚关注 / 取消关注后的更新速度。

如果 X 暂时无法返回关系状态，扩展不会把接口失败伪装成“确认未关注”；之后再次看到该账号时仍可重新核实。你也可以在设置页对误判账号使用“恢复显示”，将其加入本地放行记录。
...
- **关注账号优先保护**：以当前 X 登录态的真实关注关系为最高优先级；已关注账号即使命中公榜或规则，也不会被标记、隐藏、静音或拉黑
- **关注关系二次核实**：时间线缺少关注字段时，仅对已命中过滤条件的账号调用 X 只读关系接口；已关注结果本地缓存 24 小时、未关注结果缓存 15 分钟
- **帖子快捷原生操作**：每条帖子 Grok / 更多按钮旁提供原生静音与拉黑按钮，不依赖公榜命中；用户主动点击时允许处理已关注账号，并复用全局限速、重试与恢复队列
- **无闪动挂载**：快捷按钮随 X 虚拟列表即时挂载，修复 SPA 返回上一页时按钮延迟出现造成的布局闪动
- **本地误判恢复**：手动取消隐藏会把账号加入本地放行记录，避免该账号在其他帖子下再次被自动处理
- **简介强规则识别**：当你自然悬停头像或用户名、X 已显示资料卡时，扩展可识别反复出现的色情引流简介模板（如“入驻约 P 平台 + 入口/小号/大号”组合）。不主动模拟悬停、不遍历用户资料，也不额外请求个人简介；单个宽泛词不会独立触发
- **跨页面后台处理队列**：评论命中后先持久化任务，再执行 X 原生静音或拉黑；离开帖子、切换标签页或刷新后任务仍会恢复。手动快捷静音/拉黑也进入同一队列
- **可控重试与风控暂停**：网络或 X 服务端临时错误退避重试；登录失效、权限不足或明确风控错误会暂停队列。拉黑缺少可靠数字用户 ID 时安全降级为静音
- **队列诊断与分类管理**：设置页可按手动、正则、简介规则和公榜来源查看任务，支持重试、取消、暂停/继续和按来源清理未开始任务
- **缓存生命周期与自愈**：名单仍每 6 小时检查更新；临时检测和关注缓存每日清理。浏览器清理扩展存储后会重新同步名单并重建索引
- **隐私审计最小化**：简介规则只保存规则版本及去除网址、推广用户名后的不可逆摘要，不保存完整简介；规则升级会自动失效旧缓存
- **X 页面稳定性保护**：限制虚拟列表扫描范围、合并批量查询，并在开发版扩展重新加载导致旧脚本失效时恢复当前 X 标签页，减少滚动跳顶、空白占位、闪动和页面报错
